  ...
  return (
    <div className="equity-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Cumulative ${mode === "dollar" ? "dollar P and L" : "R multiple"} equity curve`}>
        <defs><linearGradient id="prototype-equity-area" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stopColor="#11834f" stopOpacity=".18"/><stop offset="1" stopColor="#11834f" stopOpacity=".01"/></linearGradient></defs>
        ...
        <polygon points={area} fill="url(#prototype-equity-area)"/>
...
export function LegacyReview({active,onNavigate:setActive}:{active:string;onNavigate:(view:string)=>void}) {
  const samples=useBrowserStore<Trade[]>("brontide-demo-prototype-v2",seedTrades,value=>Array.isArray(value)&&value.every(row=>row&&typeof row.symbol==="string"&&typeof row.date==="string"&&[row.pnl,row.r,row.risk,row.plannedR].every(Number.isFinite)));
  const trades=samples.value;
  ...
  return (
    <div className="legacy-review"><p className="workspace-notice">Historical prototype · synthetic sample trades. These five original tabs shared one dashboard.</p>


      <section className="workspace">
        {samples.error&&<p role="alert">{samples.error}</p>}
        <header className="topbar">
          ...
          <div className="header-actions">
            ...
            <button className="secondary-button" disabled title="This control was not implemented in the original prototype"><Icon name="settings" size={16}/> Filters</button>
