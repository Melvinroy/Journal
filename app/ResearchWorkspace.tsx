...
import { demoStorageKey, getSampleResearchJson, sampleResearchRun } from "../lib/review-demo";
...
export function ResearchWorkspace({kind,onChart,demo=false}:{demo?:boolean;kind:"scan"|"backtest";onChart:(context:MarketContext)=>void}) {
  ...
  const [compare,setCompare]=useState<string[]>([]),[legacy,setLegacy]=useState(true);
  ...
  const preferences=useBrowserStore<{symbol:string;diagnostic:boolean;sort:string;descending:boolean;metric:string}>(demoStorageKey(`brontide-research-view-${kind}-v1`,sample),{symbol:"",diagnostic:false,sort:"setup_date",descending:false,metric:"rvol"});
  ...
  return <section className="research-shell">
    ...
    {legacy?(kind==="scan"?<ScansDashboard/>:<BacktestDashboard/>):<div className="research-workspace">
      ...
      <button onClick={()=>setRetry(retry+1)}>Refresh / retry</button><button disabled={!run} onClick={download}>Export visible page + provenance</button>{run&&!sample&&<a href={`/v1/research/runs/${run.run_id}/export`} download>Download complete run / ledger</a>}</div>
      {sample&&run&&<button onClick={()=>{const url=URL.createObjectURL(new Blob([JSON.stringify(sampleResearchRun(kind),null,2)],{type:"application/json"}));const a=document.createElement("a");a.href=url;a.download="sample-research-ledger.json";a.click();URL.revokeObjectURL(url);}}>Download complete sample ledger</button>}
