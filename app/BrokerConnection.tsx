...
export function BrokerConnection({ paper, demo = false }: { paper?: PaperExecution; demo?: boolean }) {
  ...
  const label = paper.status?.connected && !paper.error ? "Paper connected" : state === "connecting" ? "Connecting to TWS" :
    state === "error" ? "Paper needs attention" : state === "blocked" ? "Paper connection blocked" : updatedAt ? "Disconnected · last known" : "TWS disconnected";
  ...
  return <section className="broker-account-summary" aria-label="IBKR account summary">
    ...
    <div className="broker-connection-detail"><strong>{paper.status?.account ?? "No verified paper connection"}</strong>
      ...
      {paper.status?.broker?.account && <p>Account equity: {paper.status.broker.account.value == null ? "Unavailable" : `${paper.status.broker.account.value.toLocaleString()} ${paper.status.broker.account.currency ?? ""}`} · Verified USD broker equity is the connected sizing basis; offline values are draft estimates.</p>}
      ...
      <div className="editor-actions"><button type="button" disabled={!paper.signedIn || paper.connecting} onClick={() => void run(paper.reconnect)}>Retry connection</button>
        ...
        <button type="button" disabled={!paper.status?.connected} onClick={() => void run(paper.disconnect)}>Disconnect</button>
        <button type="button" disabled={!paper.signedIn} onClick={() => void run(async () => { await paper.request("pause", {}); await paper.refresh(); setMessage("Entries and managed rules paused. Broker-held protection retained."); })}>Pause trading</button></div>
