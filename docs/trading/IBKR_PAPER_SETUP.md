...
6. With IBKR API Read-Only still enabled and `BRONTIDE_IBKR_SUBMISSIONS_ENABLED=false`, run `python scripts/ibkr-paper-smoke.py` and confirm it reports exactly one masked allowlisted account without submitting an order. The UI may also qualify a stock contract and request a bid/ask snapshot through the same owned client; delayed, frozen, incomplete or stale data is displayed as non-executable and cannot validate an order intent. Any missing, extra or changed account/configuration fails closed and requires reverification.

Disabling TWS Read-Only and enabling Brontide submissions are one controlled-session action performed only after the operator approves the exact batch in `IBKR_PAPER_ACCEPTANCE_BATCH.md`. Read-only connectivity, a port, a masked account pattern or a stored configuration flag is never approval. If the process restarts, the connection changes or the batch changes, stop and revalidate before any further order operation.
