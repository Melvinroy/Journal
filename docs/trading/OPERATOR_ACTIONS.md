...
## Phase 4 narrow release candidate and Phase 5 operator validation

This is preparation only. The proposed first live subset is USD whole-share stocks, long limit entries, regular hours, DAY entries, broker-held stop protection, one target and separately verified breakeven. Current paper-only configuration is not a live implementation. Changing a label, port, account binding or environment variable does not authorize or establish live support.

| Release requirement | Existing behavior / evidence boundary | Required next evidence |
| --- | --- | --- |
| Prevent new entries and managed activity | `PaperService.pause_operations()` halts the latest eligible test session and calls `disarm()`, clearing armed/reviewed authority; this is a server operation, not a claim of a new visible emergency button | Separately reviewed operational procedure, ownership and observed pause behavior; do not invoke during preparation |
| Pause one campaign | Positions' **Pause managed rules** uses `cancel-exits`; confirmation explicitly retains broker orders | Broker-observed retained stops/targets and no subsequent managed writes |
| Disconnect | Existing connection control clears armed state and marks open campaigns offline; broker-held stops are not cancelled by this path | Independently confirmed working protection and fresh reconciliation after reconnect; stale UI cannot prove a stop is working |
| Recover uncertainty | **Reconcile owned campaign** rebuilds from broker evidence and keeps managed exits paused; uncertain entries cannot be retransmitted | Complete authoritative snapshots/executions, exact owned identities, no duplicate action; then separate **Review and resume exits** if eligible |
| Close exposure | **Close with bounded limit** is bounded cleanup, not a guaranteed immediate liquidation or universal kill switch | Repaired protection lifecycle and exhausted/uncertain cleanup handling; flat, cleared orders and complete accounting separately proven |
| Enable narrow live candidate | Live mode remains blocked; paper safeguards and fixture passes alone do not implement the candidate | Exact reviewed release source, supported dependency, security/storage gates, operational alert/recovery evidence and explicit live configuration/pilot approval |

Phase 5 should reuse this document's procedures rather than create a second trading form. Complete A (paired storage diagnosis), then B (distinct-user denial), then separately approved C (cutover/rollback). D (actual expiry/clock), E (real account transition), and F (external decisions) keep their own statuses. Follow the [connected run card](IBKR_PAPER_ACCEPTANCE_QC.md#september-23-connected-rehearsal-sequence--prepared-execution-blocked) only after its independent execution boundary is legitimately resolved. User acceptance then covers ordinary planning/save/review, position management and Journal reconciliation at the exact approved source. A supervised one-share/one-campaign live pilot remains a separate final approval after applicable gates pass; broader allocation/runner/session controls stay planning-only or blocked until separately accepted.

### Current storage-path check — agent context only

Read-only inspection on September 23 still finds `Brontide`, `BrontidePrivate\Runtime` and `BrontidePrivate\Backups` as normal directories in the agent's local application-data view, with no reported reparse/link target. No folder was created and no file contents were read for this check. This does not resolve the normal/elevated desktop screenshots reporting absence: fresh paired diagnostics from section A are still required.

Source selection remains explicit: `paper_store.py:18` chooses an injected path, then `BRONTIDE_PAPER_DATABASE`, then the local application-data `Brontide\paper-lifecycle.sqlite3` default. `paper_auth.py:14` chooses `BRONTIDE_PAPER_OWNER_FILE` or the matching `Brontide\paper-owner.json` default. `ibkr_tws.py:247` requires an explicit `BRONTIDE_IBKR_VERIFICATION_FILE`. These are source rules, not proof of an active service's environment or chosen ledger. Keep active path provenance and desktop visibility **unresolved** until compared in the same observation window. Do not select/recreate an alternative directory to bypass the discrepancy.
