...
export function useBrowserStore<T>(key: string, fallback: T, validate?: (value:unknown)=>boolean) {
  ...
  useEffect(() => {
    const read = () => {
      try { const next=readStored(window.localStorage, key, fallback);if(validate&&!validate(next))throw new Error("Invalid saved data");setValue(next); setLoadedKey(key); setError(""); setReady(true); }
  ...
  const save = useCallback((next: T) => {
    if (!ready || loadedKey !== key) return false;
